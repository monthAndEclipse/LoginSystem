...
router.post('/signin',passport.authenticate('local',{customMessage:true}),
    (req,res,next)=>{
        console.log(req.customMessage)
        if(req.isAuthenticated()){
            res.end(JSON.stringify({code:0,message:req.authInfo,user:req.user.id,sessionID:req.sessionID}))
...
router.get('/isSignin',(req,res)=>{
    res.end(JSON.stringify({isSignin:req.isAuthenticated()}));
})
